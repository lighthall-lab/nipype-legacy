...
def create_connectivity_pipeline(name="connectivity"):
    ...
    get_first_image_interface = Function(input_names=["in_file"],
                             output_names=["out_file"],
                             function=get_first_image)
    get_b0 = pe.Node(interface=get_first_image_interface, name="get_b0")

    mapping.connect([(eddycorrect, get_b0,[('eddy_corrected', 'in_file')])])
    mapping.connect([(get_b0, resampleb0,[('out_file', 'in_file')])])
    ...
    mapping.connect([(get_b0, inverseROIsToB0,[('out_file', 'reference')])])
    ...
    connectivity.connect([(mapping, outputnode, [("tck2trk.out_file", "tracts"),
        ...
        ("mri_convert_ROI_scale500.out_file", "rois_orig"),
