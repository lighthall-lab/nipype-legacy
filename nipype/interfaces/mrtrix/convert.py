...
def read_mrtrix_header(in_file):
    ...
    for line in fileobj:
        if line == 'END\n':
            print 'Reached the end of the header!'
            break
        elif ': ' in line:
            line = line.replace('\n','')
            line = line.replace("'","")
            ...
            value = line.split(': ')[1]
            header[key] = value
            print '...adding "{v}" to header for key "{k}"'.format(v=value,k=key)
    fileobj.close()
...
def read_mrtrix_streamlines(in_file, header, as_generator=True):
    ...
    def points_per_track(offset):
        ...
        n_points = 0
        track_points = []
        print 'Identifying the number of points per tract...'
    ...
    def track_gen(track_points):
        ...
        while True:
            ...
            if n_streams == stream_count:
                print '100% : {n} tracks read'.format(n=n_streams)
                raise StopIteration
...
class MRTrix2TrackVis(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if isdefined(self.inputs.matrix_file) and isdefined(self.inputs.registration_image_file):
            ...
            print 'Using affine from registration image file {r}'.format(r=self.inputs.registration_image_file)
            print reg_affine
            ...
            print 'Saving transformed Trackvis file as {out}'.format(out=out_filename)
            ...
            trk.write(out_filename, trk_tracks, trk_header)
            print 'Saving Trackvis file as {out}'.format(out=out_filename)
